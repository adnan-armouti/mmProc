...
from mmproc.data.visual_data import VisualData
...
import mmproc.utils.utils_action_organizer as action_organizer
...
class FileManager():
    ...
    def read_visual_file_to_file(self) -> None:
        ...
        if (self.read_filename is not None):
            read_files = [f for f in self.files if self.read_filename in f]
        # sort read_files
        read_files.sort()
        read_files = sorted(read_files, key = len)
        # to be updated by user's own method
